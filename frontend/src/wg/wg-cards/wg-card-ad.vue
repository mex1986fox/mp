	...
	<div class="wg-card-ad">
		<div class="wg-card-ad__header">
			<div v-if="dObj.user" class="ui-avatar-block wg-card-ad__avatar-block">
				...
				<a class="ui-link ui-avatar-block__link">{{dObj.user.name+" "+dObj.user.surname}}</a>
				<div class="ui-avatar-block__title">{{parseDate(dObj.date_create)}}</div>
			</div>
			<button
				@click="showMenu=true"
			...
			<ui-menu :show="showMenu" @onHide="showMenu=false" position="left-bottom">
				<ul
					v-if="user_id!=undefined && dObj.user!=undefined && user_id==dObj.user.id"
					class="wg-card-ad__menu"
				>
					<li class="wg-card-ad__menu-li">Редактировать</li>
					<li class="wg-card-ad__menu-li">Отметить проданным</li>
					<li class="wg-card-ad__menu-li">Поднять</li>
				</ul>
				<ul v-else class="wg-card-ad__menu">
					<li @click="(showMenu=false, showContacts=true)" class="wg-card-ad__menu-li">Показать контакты</li>
					<li
		...
		<router-link
			class="ui-link wg-card-ad__link"
			to="/googlre"
		...
		<div class="row">
			<div class="col_6 col-phone_6">
				<wg-slider
					...
					:slide="JSON.parse(JSON.stringify(dObj.slide))"
					:select="numberPhoto"
			...
			<div class="col_6 col-phone_6">
				<div class="wg-card-ad__info">
					<div class="wg-card-ad__info-header">{{mascNumber(dObj.price_corret)}} руб.</div>
					...
					<span class="wg-card-ad__info-param">
						...
						<span
							class="wg-card-ad__info-value"
						>{{(fuel!=undefined?fuel.name:"--") + (dObj.engin_volume!=undefined?" "+dObj.engin_volume+" л.":"") +(dObj.engin_power!=undefined?" "+dObj.engin_power+" л.c.":"")}}</span>
					</span>
					<span class="wg-card-ad__info-param">
		...
		<div class="wg-card-ad__buttons">
			...
			<span class="wg-card-ad__counter">{{dObj.likes!=undefined?dObj.likes:"0"}}</span>
			<button
				@click="createLike('-1')"
				class="ui-button ui-button_circle ui-button_circle_mini ui-button_flat"
			>
		...
		>
			<div class="container">
				<div class="row">
					<div
						class="col_8 col_offset-2 col-tablet_10 col-tablet_offset-1 col-phone_6 col-phone_offset-0"
					>
		...
		>
			<div class="container">
				<div class="row">
					<div
						class="col_8 col_offset-2 col-nbook_10 col-nbook_offset-1 col-tablet_10 col-tablet_offset-1 col-phone_6 col-phone_offset-0"
					>
						<wg-messanger :apponent="dObj.user.id" @onHide="showMessenger=false"></wg-messanger>
					</div>
				</div>
			</div>
		</ui-blind>
